...
const Product = ({ product }: ProductProps) => {
  ...
  return (
    <>
      ...
      <Card className="group relative rounded-xl w-full h-full overflow-hidden border border-none bg-transparent hover:bg-white shadow-none hover:shadow-lg hover:border-gray-300 transition-all duration-300 ease-in-out flex flex-col p-0">
        {" "}
        ...
        >
          <div className="relative w-full h-full">
            <Image
              ...
              alt={product.name}
              priority
              className="object-cover w-full h-full transition-all duration-300 group-hover:scale-105 rounded-xl"
              onMouseEnter={() => setIsImageHovered(true)}
              onMouseLeave={() => setIsImageHovered(false)}
            />
            {/* Plus icon button at bottom-right of image, only if not in cart or quantity is 0 */}
            {(!productInCart || productInCart.quantity === 0) && (
              <Button
                className="absolute bottom-3 right-3 z-30 rounded-full bg-white text-gray-700 shadow-md p-0 w-8 h-8 flex items-center justify-center hover:bg-gray-100 border border-gray-300"
                size="icon"
                variant="ghost"
            ...
            {/* Cart controls at bottom-right of image if in cart and quantity > 0 */}
            {productInCart && productInCart.quantity > 0 && (
              <div
                onClick={(e) => e.stopPropagation()}
                className="absolute bottom-3 right-3 z-40 flex items-center bg-white border border-gray-300 rounded-full shadow-md px-2 py-1 gap-1"
                style={{ boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}
              >
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-7 w-7 p-0 text-primary bg-white rounded-full border border-primary"
                  onClick={handleDecrementWithToast}
                  aria-label="Decrease quantity"
                >
                  <Minus className="h-4 w-4 text-primary" />
                </Button>
                <span className="mx-1 min-w-[1.5rem] h-full flex items-center justify-center text-center font-medium text-primary text-sm">
