...
const FooterSections = memo(({ store }: { store: StoreFooter }) => {
  ...
  // Removed unused linkListVariants
  ...
  return (
    ...
    >
      {store?.footerSections
        ?.filter((section) => section.sectionName !== "Social Media")
        .map((footerMenu: FooterSection, i: number) => (
          <motion.div key={i} variants={itemVariants}>
            <h3 className="mb-(spacing:--footer-heading-margin) text-(size:--footer-heading-size) font-(weight:--footer-heading-weight) tracking-tight font-(family-name:--font-secondary)">
              ...
              {footerMenu.sectionItems.map(
                (sectionItem: FooterMenuItem, ind: number) => (
                  ...
                  >
                    <Button
                      variant="link"
                      className="text-xs sm:text-sm hover:text-(color:--footer-link-hover-color) transition-colors justify-start font-(family-name:--font-primary) p-0 h-auto"
                    >
      ...
      <div className="container px-4 sm:px-6 md:px-8 max-w-7xl">
              ...
              <CardHeader className="px-0 pb-4 sm:pb-6">
                ...
                >
                  <Image
                    ...
                    priority
                  />
                  <span className="text-lg bg-transparent hover:bg-transparent sm:text-xl font-(weight:--font-bold) font-(family-name:--font-secondary)">
                    {name}
                  </span>
                </Link>

                <CardDescription className="mt-2 text-sm sm:text-base font-(family-name:--font-primary) wrap-anywhere">
        ...
        >
          <motion.div
            ...
            variants={sectionVariants}
          >
            <p className="text-xs sm:text-sm text-(color:--primary) font-(family-name:--font-primary) text-center md:text-left">
          ...
          <motion.div
            ...
            variants={sectionVariants}
          >
            <p className="text-xs sm:text-sm text-(color:--primary)/70 font-(family-name:--font-primary) flex items-center gap-1.5 justify-center ">
              Powered by{" "}
              <Link
