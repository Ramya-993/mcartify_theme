...
export default function TermsAndConditions() {
  ...
  const getTermsAndConditions = async () => {
    try {
      const response = await AxiosFetcher.get("/stores/terms-and-conditions")
      return response?.data
    } catch (e) {
      setError(e?.response?.data?.Message || "Failed to load terms and conditions")
        ...
        <Card className="max-w-4xl mx-auto border-red-200">
          ...
          <CardContent>
            <Alert variant="destructive">
              ...
              <AlertDescription>
                {data?.Message || error || "Something went wrong while loading the terms and conditions."}
              </AlertDescription>
            </Alert>
          </CardContent>
        </Card>
