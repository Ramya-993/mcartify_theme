...
export default function ShippingPolicy() {
  ...
  const getShippingPolicy = async () => {
    try {
      const response = await AxiosFetcher.get("/stores/shipping-policy");
      return response?.data;
    } catch (e) {
      setError(e?.response?.data?.Message || "Failed to load shipping policy");
          ...
          <CardContent>
            <Alert variant="destructive">
              ...
              <AlertDescription>
                {data?.Message ||
                  error ||
                  "Something went wrong while loading the shipping policy."}
              </AlertDescription>
            </Alert>
          </CardContent>
        </Card>
      ...
      <Card className="max-w-4xl mx-auto border-none shadow-none bg-transparent">
        ...
        <CardContent>
          <div
            ...
            dangerouslySetInnerHTML={{ __html: data?.ShippingPolicy }}
          />
        </CardContent>
      </Card>
    </div>
