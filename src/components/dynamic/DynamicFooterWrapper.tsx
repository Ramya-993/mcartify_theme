"use client";

import React from "react";

import ErrorBoundary from "../ErrorBoundary";
import { useDynamicComponent } from "@/hooks/useDynamicComponent";

interface DynamicFooterWrapperProps {
  name: string;
  image: string;
}

const DynamicFooterWrapper = ({ name, image }: DynamicFooterWrapperProps) => {


  const fallbackComponent = (
    <div className="w-full">
      <div className="bg-white p-4 rounded-lg shadow-lg">
        <h1 className="text-2xl font-bold text-red-500">
          Error loading Footer
        </h1>
        <p className="text-sm text-gray-600">
          Please check your theme configuration and try again.
        </p>
      </div>
    </div>
  );
  // Memoize the dynamic component to prevent recreation on re-renders
  const DynamicFooter = useDynamicComponent("Footer")

  return (
    <ErrorBoundary
      fallback={fallbackComponent}
      onError={(error) => console.error("Error in DynamicFooter:", error)}
    >
      <DynamicFooter name={name} image={image} />
    </ErrorBoundary>
  );
};

export default DynamicFooterWrapper;
